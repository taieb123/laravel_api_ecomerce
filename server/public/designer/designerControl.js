  ...
  _init:function()
  {
    ...
    if($('#site_name').length>0 && $('#site_name').val() == 'admin'){
      if($('#hf_base_url').length>0 && $('#lang_code').length>0){
        $.getJSON( $('#hf_base_url').val() + '/public/lang/'+ $('#lang_code').val() +'/admin_js.json', function( data ) {
      ...
      if($('#hf_base_url').length>0 && $('#lang_code').length>0){
        $.getJSON( $('#hf_base_url').val() + '/public/lang/'+ $('#lang_code').val() +'/frontend_js.json', function( data ) {
          localizationString = data;
        });
      }
    }
